...
def handler(context, inputs):
    ...
    if systemType == 'ceph':
        data = aa.runOrchAction(projectId,"com.gvp.bp.nfs/nfsProvision",{"nfsName" : name, "size": size})
        
        inputs['volumeName'] = get_label_by_value(data,'volumeName')
        inputs['subvolumeName'] = get_label_by_value(data,'subvolumeName')
        inputs['pusedo'] = get_label_by_value(data,'pusedo')
        inputs['accessType']  = get_label_by_value(data,'accessType')
        inputs['squash']  = get_label_by_value(data,'squash')
        inputs['clients']  = get_label_by_value(data,'clients')
        inputs['exportId'] = get_label_by_value(data,'exportId')
        inputs['nfsClusterId'] = get_label_by_value(data,'nfsClusterId')
        inputs['usage'] = get_label_by_value(data,'usage')
    elif systemType == 'vsan':
        data = aa.runOrchAction(projectId,"com.gvp.bp.vsanfs/fileShareCreate",{"volumeName" : name, "size": size})
        
        inputs['fileShareUuid'] = get_label_by_value(data,'uuid')
        inputs['pusedo'] = get_label_by_value(data,'pusedo')
        inputs['accessType']  = get_label_by_value(data,'accessType')
        inputs['squash']  = get_label_by_value(data,'squash')
        inputs['clients']  = get_label_by_value(data,'clients')
        inputs['usage'] = get_label_by_value(data,'usage')
        inputs['endpoint'] = get_label_by_value(data,'endpoint')
        inputs['endpointCluster'] = get_label_by_value(data,'endpointCluster')
