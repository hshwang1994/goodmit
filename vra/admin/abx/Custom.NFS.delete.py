...
def handler(context, inputs):
    if inputs.get('attachedVm'): raise Exception('NFS has Attached VM')
    aa = AaManager(context)
    projectId = inputs['__metadata']['project']
    systemType = inputs['systemType']
    if systemType == 'ceph':
        data = aa.runOrchAction(projectId,"com.gvp.bp.nfs/nfsRemoval",{
            "exportId" : inputs['exportId'], 
            "subvolumeName": inputs['subvolumeName'],
            "nfsClusterId": inputs['nfsClusterId'],
            "volumeName": inputs['volumeName']
            
        })
    elif systemType == 'vsan':
        data = aa.runOrchAction(projectId,"com.gvp.bp.vsanfs/fileShareDelete",{
            "uuid" : inputs['fileShareUuid'],
            "endpoint": inputs['endpoint'],
            "endpointCluster": inputs['endpointCluster']
        })
